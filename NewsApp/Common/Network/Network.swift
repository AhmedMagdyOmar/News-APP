...
extension Network: NetworkProtocol {
    func request<T>(_ request: URLRequestConvertible, decodeTo type: T.Type, completionHandler: @escaping NetworkCompletion<T>) where T : BaseCodable {
        ...
        #endif
        session.request(request).responseJSON {[weak self] response in
                ...
                for (key, value) in request.parameters ?? [:] {
                    if let value = value as? [String: Any] {
                        for (key2, value2) in value {
                            multipartFormData.append("\(value2)".data(using: .utf8)!, withName: "\(key)[\(key2)]")
                        }
                    } else if let arrayValue = value as? [[String: Any]] {
                        for index in arrayValue.indices {
                            for (key2, value2) in arrayValue[index] {
                                multipartFormData.append("\(value2)".data(using: .utf8)!, withName: "\(key)\([index])[\(key2)]")
                                print("the x is \("\(key)\([index])[\(key2)]")")
                               }
                           }
                    }
                    else {
